...
typedef struct 
{
	...
	unsigned long long child_freqs[6];
	unsigned long long revbwt_start;
...
void SLT_execute_iterator(SLT64_iterator_t * SLT_iterator)
{
	unsigned long long curr_stack_size=min_SLT_stack_size;
	unsigned long long curr_stack_idx=0;
	SLT64_params_t SLT_params;
	unsigned long long string_depth;
	unsigned long long char_pref_counts[28];
	unsigned long long last_char_pref_counts[7];
	unsigned long long pref_count_query_points[7];
	unsigned long long last_char_pref_count;
	unsigned int i,j,k;
	unsigned int extension_exists;
	unsigned int nchildren;
	unsigned int includes_EOT_char;
	unsigned long long last_char_freq;
//	unsigned int right_extension_exists;
	unsigned int npref_query_points;
	unsigned long long interval_size;
	unsigned long long max_interval_size;
	unsigned int nexplicit_WL;
	unsigned int max_interval_idx;
	unsigned long long ntraversed_nodes=0;
	unsigned long long revbwt_start;
	Basic_BWT64_t * BBWT=SLT_iterator->BBWT;
...
//				printf("query point number %d is %d\n",j,pref_count_query_points[j]);
		...
		{
			npref_query_points--;
			DNA5_multipe_char_pref_counts64(BBWT->indexed_BWT,npref_query_points,

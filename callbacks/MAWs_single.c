...
static void initLeftRightFreqs(RightMaximalString_t rightMaximalString, MAWs_callback_state_t *state) {
	...
	for (i=1; i<5; i++) {
		...
		frequency=0;
		for (j=0; j<6; j++) frequency+=rightMaximalString.frequency_leftRight[i][j];
	...
	for (j=1; j<5; j++) {
		...
		frequency=0;
		for (i=0; i<6; i++) frequency+=rightMaximalString.frequency_leftRight[i][j];
...
void MAWs_callback(RightMaximalString_t rightMaximalString, void *applicationData) {
	...
	for (i=1; i<5; i++) {
		char_mask1<<=1;
		if (!(rightMaximalString.leftExtensionBitmap&char_mask1)) continue;
		char_mask2=1;
		for (j=1; j<5; j++) {
			char_mask2<<=1;
			if ( !(rightMaximalString.rightExtensionBitmap&char_mask2) ||

...
unsigned int BWT_load_callback (unsigned char ** _buffer, void * intern_state)
{
	unsigned int len;
...
//	printf("We are going to call the callback\n");
	ret_code=load_Basic_BWT_from_callback(_BBWT,len,primary_idx,BWT_load_callback,&state);
	free(state.buffer);
	fclose(state.BWT_file);
	if(ret_code!=0)
	{
...
// Build a BWT index on the text. 
	BBWT1=Build_BWT_index_from_text(text,textlen,Basic_bwt_no_free_text);
	...
	if((err_code=cmp_BBWTs(BBWT1,BBWT2)))
